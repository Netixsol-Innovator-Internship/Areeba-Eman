...
import { addMessage, resetChat, setChatId, setLoading, ChatMessage, ChatProduct } from "../features/chat/chatSlice";
...
export default function ChatWidget() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (resp.chatId) dispatch(setChatId(String(resp.chatId)));

      const aiText = resp.reply || resp.explanationai || 'No response';
      const aiMsg: ChatMessage = { id: genId(), sender: 'ai', text: aiText, products: resp.products };
                      ...
                      <ul className="space-y-1 text-sm">
                        {m.products.map((p, idx) => (
                          <li key={idx}>
                            <Link
                              href={`/products/${p._id}`}
                              className="flex justify-between items-center bg-white rounded-md p-2 shadow-sm border hover:shadow-md transition hover:bg-gray-50"
                            >
                              <span>
                                {p?.name}{' '}
                                {p?.brand && (
