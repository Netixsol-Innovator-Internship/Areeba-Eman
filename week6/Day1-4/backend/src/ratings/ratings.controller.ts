...
export class RatingsController {
  ...
  add(@Param('productId') pid: string, @Body() body: CreateRatingDto, @Req() req: any) {
    const userId = req.user.sub;
    return this.ratings.add(pid, userId, body.stars, body.comment || '');
  }
  ...
  @Get()
findAll() {
  return this.ratings.findAll(); // fetch all ratings
}
  ...
  // NEW route
  @Get(':productId/list')
  list(@Param('productId') pid: string) {
    return this.ratings.list(pid);
  }
}
