...
import { useState } from 'react'
import { useGetProductsQuery, useSetProductSaleMutation } from '@/features/api/apiSlice'
...
export default function StartSalePage() {
  ...
  const handleStartSale = async () => {
    ...
    if (saleEnd) payload.saleEnd = new Date(saleEnd).toISOString()

    try {
      await setProductSale({ id: selectedProduct, body: payload }).unwrap()
      alert('Sale started!')
    } catch (err) {
      console.error('Failed to start sale:', err)
      alert('Failed to start sale: ' + JSON.stringify(err.data || err))
